...
angular.module('cities').controller('LoginController', ['$http', '$scope', '$window', '$cookies', '$rootScope','$location', function ($http, $scope, $window, $cookies, $rootScope,$location) {
    ...
    var urlKDC = 'https://localhost:8085/api';
    var urlServer = 'https://localhost:8080/api/user';
            ...
            function (inputValue) {
                ...
                if (inputValue) {
                    ...
                    $http.post(urlServer + '/register/anonimous', {
                        ...
                        swal({
                                title: "Password",
                                text: "Write your anonimous password",
                                type: "input",
                                showCancelButton: true,
                                closeOnConfirm: false,
                                animation: "slide-from-top",
                                        ...
                                        $http.get('https://localhost:8085/ttp/publication').success(function (res) {
                                            if (username === res.username && password === password) {
                                                compareHash(res, function () {
                                                    swal("Well done!", "Your user is registered and you can log in!", "success");
                                                    $window.location.href= "https://localhost:8080/#/anonimous/login"
                                                })
                                            }
                                        })
                                    }).error(function (res) {
    ...
    $scope.register = function () {
        var user = {
            socialReason: $scope.user.reason,
            CIF: sha256($scope.user.cif)
        };
        $http.post(urlServer + '/register', user).success(function (res) {
            $http.get('https://localhost:8080/api/publickey').success(function (response) {
                ...
                $http.post('https://localhost:8080/api/sign', {data: publicBlinded}).success(function (res) {
                    ...
                    $http.post('https://localhost:8080/api/sign/verify', {data: unblind.toString()}).success(function (res) {
                        var data = {
                        ...
                        $http.post('https://localhost:8080/api/challenge', {data: data}).success(function (res) {
                            console.log('Token: ' + unblind.toString(16));
                            swal({
                                    title: "Verifing  user",
                                    text: "Continue to verify that you are a real user",
                                    type: "info",
                                ...
                                function () {
                                    setTimeout(function () {
                                        swal({
                                                title: "User verified",
                                                text: 'Now you can register anonimously',
            ...
            /*swal("Good job!", "User registered correctly! Now you can go anonimously", "success");
             $scope.user = null;
             $cookies.put('canLogin', 'true');
             $window.location.href= "https://localhost:8080/#/anonimous/login";*/
        }).error(function (res, status) {
            swal({
                title: "oops...",
                text: "This user is also registered!",
                type: 'error',
                timer: 3000,
    ...
    /*$scope.login = function () {
        var loginID = sha256($scope.user.id);
        var login = {
            login: loginID,
            password: $scope.user.password
        };
        $http.post(urlKDC + '/login', login).success(function (res, status) {
            swal("Good job!", "Login correct!", "success");
            $scope.user = null;
        }).error(function (res, status) {
            if (status == 400)
            ...
            swal({
                title: "Oops...",
                text: status == 400 ? 'Bad password' : 'User does not exist',
                type: 'error',
                timer: 3000,
                showConfirmButton: true
            });
